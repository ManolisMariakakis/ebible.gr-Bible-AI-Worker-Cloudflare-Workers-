...
- [ebible.gr — Bible AI Worker (Cloudflare Workers)](#ebiblegr--bible-ai-worker-cloudflare-workers)
  - [Features](#features)
  ...
  - [Usage Examples](#usage-examples)
    - [cURL](#curl)
    - [Browser (Fetch API)](#browser-fetch-api)
  - [System Prompt (Rules)](#system-prompt-rules)
  - [Reference Rules \& Abbreviations](#reference-rules--abbreviations)
  - [Architecture / Flow](#architecture--flow)
  - [Errors \& Troubleshooting](#errors--troubleshooting)
  - [Security Notes](#security-notes)
  - [Complete Source Code (worker.js)](#complete-source-code-workerjs)
  - [Sample `wrangler.toml`](#sample-wranglertoml)
  - [License](#license)
...
Set your secrets in the Worker:
...
curl -X POST "https://<your-worker>/api/chat" \
  ...
  -d '{"message":"Show me Psalm 23:1"}'
```

### Browser (Fetch API)
```js
const res = await fetch("https://<your-worker>/api/chat", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ message: "John 3:16" }),
});
const { reply } = await res.json();
document.querySelector("#answer").innerHTML = reply; // Already HTML
```
...
- Do **not** expose your `OPENAI_API_KEY` to clients; Worker handles requests server-side.  
- Current config allows CORS from all origins (`*`). Restrict to your domain in production.  
- Model: `gpt-4o-mini` (configurable).  
- System prompt enforces:
  - HTML answers  
  - Bible-first scope  
  - Auto-rejects unrelated queries  
...
```toml
name = "ebible-gr-bible-ai-worker"
main = "worker.js"
...
> Add your secrets using `wrangler secret put OPENAI_API_KEY` (and optionally `WEBHOOK_URL`).
